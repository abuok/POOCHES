...
export interface TradingStatistics {
  ...
  bestTrade: number;     // best single trade PnL
  worstTrade: number;    // worst single trade PnL
  consecutiveWins: number; // max consecutive wins
  consecutiveLosses: number; // max consecutive losses
  profitFactor?: number;   // wins total / |losses total|
 ...
 */
export function calculatePnL(trade: TradeEntry): number {
  // Handle both old format (entry/exit/lots) and new format (entryPrice/exitPrice/size)
  const entry = trade.entryPrice || (trade as any).entry || 0;
  const exit = trade.exitPrice || (trade as any).exit || 0;
  const size = trade.size || (trade as any).lots || 0;
  
  if (!exit || !entry || !size) return 0;
 ...
 */
export function getTodayTrades(trades: TradeEntry[]): TradeEntry[] {
  const today = getTodayString();
  return trades.filter(t => {
    // Handle both old format (t.date) and new format
    const tradeDate = t.date || (t as any).date;
 ...
 */
export function calculateTotalPnL(trades: TradeEntry[]): number {
 ...
 */
export function calculateTodayPnL(trades: TradeEntry[]): number {
 ...
 */
export function getTradingStatistics(trades: TradeEntry[]): TradingStatistics {
  // Handle empty trades
  if (!trades.length) {
    return {
      totalTrades: 0,
  ...
  trades.forEach(trade => {
    ...
    const outcome = (trade as any).outcome || (pnl > 0 ? 'WIN' : pnl < 0 ? 'LOSS' : 'BREAKEVEN');
 ...
 */
export function formatStatistics(stats: TradingStatistics, currency: string = '$'): Record<string, string> {
  return {
